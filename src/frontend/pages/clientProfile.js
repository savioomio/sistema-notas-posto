...
async function generatePDF() {
  ...
  try {
    ...
    // Logo (você precisará adicionar uma imagem real)
    // doc.addImage('/logo.png', 'PNG', 10, 10, 40, 20);
    
    // Cabeçalho
    doc.setFontSize(20);
    doc.text('Sistema do Posto', 105, 20, { align: 'center' });
    doc.setFontSize(16);
    ...
    invoices.forEach(invoice => {
      yPos += 7;
      if (yPos > 280) { // Nova página se necessário
        doc.addPage();
        yPos = 20;
      }
      
      doc.text(formatDate(invoice.purchase_date), 10, yPos);
      doc.text(formatDate(invoice.due_date), 50, yPos);
      doc.text(formatCurrency(invoice.total_value), 90, yPos);
      ...
      doc.text(invoice.status === 'paga' ? 'Paga' : (isOverdue(invoice.due_date) ? 'Vencida' : 'Pendente'), 130, yPos);
